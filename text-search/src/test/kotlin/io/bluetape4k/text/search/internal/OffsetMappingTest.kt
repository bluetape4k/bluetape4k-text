...
class OffsetMappingTest {
    ...
    fun `NFC - 자모 분리 입력의 길이 감소 매핑 검증`() {
        // 준비: ᄀ(U+1100) + ᅡ(U+1161) + X → NFC → 가(U+AC00) + X (3 chars → 2 chars)
    ...
    fun `NFKC - 합성 기호 길이 증가 매핑 검증`() {
        // 준비: A + ㈜(U+321C) + B → NFKC → A(주)B (3 chars → 5 chars)
    ...
    fun `빈 문자열, 단일 char, ascii-only - 정규화 무영향 케이스`() {
        ...
        // 준비, 실행, 검증: 순수 ascii. 정규화로 길이 변화 없음(identity 동작)
    ...
    fun `end-inclusive round-trip - NFKC 매치의 원본 substring 복원 검증`() {
        ...
        // 실행: normalized에서 "(주)" 매치 가정 — start=1, end=3 (inclusive)
        val normStart = 1
