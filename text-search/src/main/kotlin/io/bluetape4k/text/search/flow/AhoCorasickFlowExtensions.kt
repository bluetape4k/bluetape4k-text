 ...
 * 기본 겹침/단어 경계 옵션에서는 trie traversal이 match를 찾는 즉시 전송하며, raw traversal 순서를
 * 유지합니다. `take(N)`을 통한 조기 종료는 모든 match를 materialize하지 않고 producer를 중단합니다.
 *
 * [SearchOptions.allowOverlaps]가 `false`이거나 [SearchOptions.wordBoundary]가 [WordBoundary.NONE]이 아니면
 * automaton은 전체 결과 후처리를 적용해야 합니다. 이 경우 이 함수는 필터링되고 eager 정렬된 [parseText]
 * 결과를 emit하되 send 사이에서 취소를 계속 확인합니다.
 *
